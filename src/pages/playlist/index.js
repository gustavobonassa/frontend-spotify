...
class Playlist extends Component {
  ...
  componentDidMount() {
    this.setState({
      userId: JSON.parse(localStorage.getItem("@Spoti:user")).id,
    });
  ...
  handleClick = async (e, data) => {
    ...
    var index = playlist.songs.findIndex(
      (f) => f.id === parseInt(data.target.className)
    );
    if (index !== -1) {
      this.setState({
    ...
    return (
      <>
        <Container style={{ position: "relative" }}>
          <Background image={playlist.thumbnail} />
          <PlayListInfo>
            <Header>
...
const mapDispatchToProps = (dispatch) =>
  bindActionCreators(
    { ...PlaylistDetailsActions, ...PlayerActions, ...SongActions },
    dispatch
  );
